...
export function updateScore(joueur) {
  ...
  setInterval(() => {
    ...
    for (let elem of joueur.values) {
      joueur.stats.prodPerSec += elem.amount * elem.multiplier;
      joueur.stats.prodPerSecWithBonus = joueur.stats.prodPerSec + joueur.stats.addedValue;
    ...
    for (let i = 0; i < joueur.upgrades.length; i++) {
      if (joueur.values[joueur.upgrades[i].target].amount >= joueur.upgrades[i].condition) {
